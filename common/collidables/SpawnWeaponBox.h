...
public:
    ...
    virtual CollidableType getType() const override { return CollidableType::SpawnWeaponBox; }

    virtual bool onCollision([[maybe_unused]] Collidable& other) override { return false; }
    ...
    virtual void set_weapon(std::unique_ptr<Weapon> set_weapon) { weapon = std::move(set_weapon); }

    virtual void clear_weapon() { weapon.reset(); }

    virtual WeaponType get_weapon_type() { return weapon ? weapon->get_type() : WeaponType::None; }

    virtual bool has_weapon() { return weapon != nullptr; }
